...
Before making your first API request, you need to pick which API client you will use. In addition to good ol' cURL HTTP requests, Arguflow offers clients for JavaScript.
...
```bash {{ language: 'js' }}
# Docs will simply provide examples for using fetch0
fetch(https://api.arguflow.com/foo)
```
...
After picking your preferred client, you are ready to make your first call to the Protocol API. Below, you can see how to send a GET request to the Conversations route to get a list of all your conversations. In the cURL example, results are limited to ten conversations, the default page length for each client.
...
<div className="not-prose">
  <Button
    ...
    variant="text"
    arrow="right"
    children="Read the docs for the Conversations route"
  />
</div>
...
- [Check out the Conversations route](/conversations)
