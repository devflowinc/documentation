...
1. Go to the Vault client at [https://vault.arguflow.com](https://vault.arguflow.com) and click the register button.
...
export const getAuthCookie = async () => {
  const api_endpoint = process.env.API_ENDPOINT || "https://api.arguflow.com/api";
...
```js {{ title: 'JavaScript' }}
const response = await fetch(`https://api.arguflow.com/api/auth`, {
  method: "GET",
  headers: {
    "Content-Type": "application/json",
