...
Users are the authors of chunks and owners of collections. On this page, we’ll dive into the different verification routes you can use to manage Users programmatically. We'll look at how to create verifications. {{ className: 'lead' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl -L 'http://api.trieve.ai/api/user' \
...
## Get the chunks and votes for a user {{ tag: 'GET', label: '/user/:user_id/:page' }}

<Row>
  <Col>

    This route allows you to get the chunks and votes for a user, as well as other metadata about the user.

    ### Response

    If the response is successful it will return a 200 status code with an array of chunks and metadata about the user.
    ...
    <Properties>
      ...
      <Property name="total_chunks_created" type="integer">
        The number of chunks a user has created
      </Property>
      <Property name="chunks" type={<text>[<a href="chunk#the-chunk-model">Chunk</a>]</text>}>
        An array of user-created chunks.
      </Property>
    </Properties>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl --request GET \
        --url https://api.trieve.ai/api/user/c9379ab0-389d-4204-8602-5ba1bf99d441/1 \
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl --request GET \
        --url https://api.trieve.ai/api/user/files/c9379ab0-389d-4204-8602-5ba1bf99d441 \
        --cookie ai-editor=your_cookie_value_here
    ```
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl --request GET \
        --url https://api.trieve.ai/api/top_users/1 \
        --cookie ai-editor=your_cookie_value_here
    ```
