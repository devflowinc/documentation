export const description =
  'In this guide, we’ll look at how Arguflow handles duplicates to achieve maximal-marginal-relevance (MMR).'

# Card duplicates

One of the stand-out features of Arguflow is how we detect and handle duplicates. In this guide, we’ll look at how Arguflow handles duplicates to achieve maximal-marginal-relevance (MMR). {{ className: 'lead' }}
...
We create the `card_collisions` SQL table in [this migration](https://github.com/arguflow/arguflow/blob/main/server/migrations/2023-06-15-202339_create-card-collision-table/up.sql). There are a couple of sub-sequent migrations to edit the table, but the core of the table is created in that migration.

The CardCollisions model is then defined in [this file](https://github.com/arguflow/arguflow/blob/main/server/src/data/models.rs#L354C1-L374) as shown below:
...
The implementation of this logic can be found in [card_handler.rs here](https://github.com/arguflow/arguflow/blob/main/server/src/handlers/card_handler.rs#L211-L323).
