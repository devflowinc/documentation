...
The team at Arguflow has been working with text embedding models and LLM's since they day they came out. Literally for the longest time possible. Leveraging that experience, we have come to the conclusion that search should be done before generation. {{ className: 'lead' }}
...
It is for this option that we ship our default [chat UI](https://github.com/arguflow/arguflow/tree/main/chat) with the fully-managed RAG system. This is because we believe that for the below average user, fully-managed RAG is the best option. It is the easiest to use and requires the least amount of effort.

If you host Arguflow, you will get the fully-managed RAG system along with the chat UI for free. You can use it if you want, or you can use the unmanaged RAG system. It is up to you and your users.

Headlessly, you can use the fully-managed RAG system by creating a retrieval-augmented topic with the [create topic route](https://redoc.arguflow.ai/redoc#tag/topic/operation/create_topic) and [message endpoint](https://redoc.arguflow.ai/redoc#tag/message/operation/create_message_completion_handler).

To do search before RAG headlessly, you will want to use the [generate off cards route](https://redoc.arguflow.ai/redoc#tag/card/operation/generate_off_cards) in combination with the [search route](https://redoc.arguflow.ai/redoc#tag/card/operation/search_card).
