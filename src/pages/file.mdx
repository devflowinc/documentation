...
Files are where most of the chunks on Vault come from. These are stored on the server in an s3 bucket. On this page, we’ll dive into the different conversation routes you can use to manage files programmatically. We'll look at how to create, update, and delete files. {{ className: 'lead' }}

## The file model

The conversation model contains all the information about the files you upload. They can be linked to a specific collection and you can access the file itself from each chunk in it. 
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl -L 'https://api.trieve.ai/api/file/9a981834-b12a-49cc-afec-4ce88a2b2456' \
...
<Row>
  <Col>
    ...
    <Properties>
      <Property name="tag_set" type="string">
        The tags you want assocaiated with the chunks in the file.
      </Property>
    </Properties>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl -L 'http://api.trieve.ai/api/file' \
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl http://api.trieve.ai/api/file/9a981834-b12a-49cc-afec-4ce88a2b2456 \
      -H 'Cookie: ai-editor=your_cookie_value_here' 
    ```
