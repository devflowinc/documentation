...
A vote is how the community can provide feedback on the quality and relevance of a chunk. On this page, we will dive into the different vote routes you can use to manage votes programmatically. {{ className: 'lead' }}
...
## Vote on a chunk {{ tag: 'POST', label: '/api/vote' }}

<Row>
  <Col>

    This route allows you to add an upvote or downvote to a chunk. (Must be authenticated to use)

    ### Required attributes
    <Properties>
      <Property name="chunk_metadata_id" type="string">
        The chunk you want to vote on.
      </Property>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
      curl --location --request POST 'https://api.trieve.ai/api/vote/' \
        --header 'Content-Type: application/json' \
        --header 'Cookie: ai-editor=your_cookie_value_here' \
        --data '{
            "chunk_metadata_id": "115672bd-02e9-4f7d-986f-f462c95dcdbc",
            "vote": true
        }'
    ```
...
## Delete a Vote on a chunk {{ tag: 'DELETE', label: '/vote/:id' }}

<Row>
  <Col>

    This route allows you to delete the upvote or downvote on a chunk that you made. (Must be authenticated to use)
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X DELETE https://api.trieve.ai/api/vote/115672bd-02e9-4f7d-986f-f462c95dcdbc \
      --header 'Cookie: ai-editor=your_cookie_value_here'
    ```
    </CodeGroup>
